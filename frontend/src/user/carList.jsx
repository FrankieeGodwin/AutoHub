...
export default function CarList() {
  ...
  const handlePageChange = (page) => {
    ...
    const carsPerPageMain = 8; // Number of cars per page
    const indexOfLastMainCar = mainCurrentPage * carsPerPageMain;
    const indexOfFirstMainCar = indexOfLastMainCar - carsPerPageMain;
    const currentMainCars = filteredCars.slice(indexOfFirstMainCar, indexOfLastMainCar);
    const totalMainPages = Math.ceil(filteredCars.length / carsPerPageMain);

    const handleMainPageChange = (page) => {
      if (page < 1 || page > totalMainPages) return;
  ...
  >
    &#9776; Filters
        ...
        <div
  className={`fixed inset-0 flex justify-start z-50 transition-transform duration-300 ${
...
>
      ...
      >
        ...
        <div className="w-full">
          ...
          ) : currentMainCars.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
              {currentMainCars.map((car) =>
                car.userId !== userId ? (
                  <div
                    key={car._id}
        ...
        {/* Pagination for Main Car Grid */}

</div>
<div className="flex justify-center mt-10 mb-10">
  <div className="inline-flex items-center space-x-2 bg-white shadow-md rounded-2xl px-4 py-2">
    <button
      onClick={() => handleMainPageChange(mainCurrentPage - 1)}
    ...
    {Array.from({ length: totalMainPages }, (_, i) => (
      <button
        key={i}
        onClick={() => handleMainPageChange(i + 1)}
    ...
    <button
      onClick={() => handleMainPageChange(mainCurrentPage + 1)}
      disabled={mainCurrentPage === totalMainPages}
      className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${
        mainCurrentPage === totalMainPages
          ? "bg-gray-200 text-gray-400 cursor-not-allowed"
          : "bg-purple-800 text-white hover:bg-purple-900"
      }`}
    >
  ...
  {Array.from({ length: totalPages }, (_, i) => (
    <button
      ...
      className={`px-3 py-1 rounded ${
        currentPage === i + 1 ? "bg-purple-800 text-white" : "bg-gray-200"
      }`}
    >
      {i + 1}
    </button>
