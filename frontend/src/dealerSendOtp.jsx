...
export default function SendOtpDealer() {
...
console.log(dealerName, email, phone, businessLicense, gstNumber, password , API_BASE);
  ...
  const handleVerify = async () => {
    ...
    if (otpInput === parsed.otp.toString()) {
      try {
        // ✅ Create dealer instead of user
        const response = await axios.post(`${API_BASE}/dealers/`, {
        ...
        setMessage("OTP verified and dealer created successfully!");
        localStorage.removeItem(`otp_${email}`);
        setCanResend(false);

        setTimeout(() => {
          navigate("/dealerLogin"); // redirect to login after creation
        }, 2000);
      } catch (error) {
        console.error("Dealer creation error:", error);
        setMessage("OTP verified but failed to create dealer.");
      }
    } else {
